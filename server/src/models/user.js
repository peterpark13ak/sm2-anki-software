...
class UserClass {
  ...
  static new(body) {
    const { name, email, password, membership } = body;
  ...
  static authenticateUser(userEmail, password) {
    return this.findOne({ email: userEmail })
      ...
      .then((user) => {
        ...
        const { _id, name, email, membership, prefs } = user;
        if (userEmail == process.env.ADMIN_ID && password == process.env.ADMIN_PW) {
          return { _id, name, email, prefs, membership: 0 };
        }
        if (!this.validPassword(password, user)) {
          return Promise.reject(new Error('Invalid User'));
        }
        return { _id, name, email, membership, prefs };
      });
  }
