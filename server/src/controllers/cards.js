...
module.exports.findCard = async (req, res, next) => {
  try {
    ...
    const card = await Card.findOne({ _id: id }).populate('deck');
...
module.exports.deleteCard = async (req, res, next) => {
  try {
    ...
    const response = await Card.deleteOne({ _id: id, user: req.user });
...
module.exports.findCardsByTag = async (req, res, next) => {
  try {
    const { tag } = req.query;
    await Joi.validate(req, cardSchemas.findCardsByTag, { allowUnknown: true });
