...
class Review extends Component {
  ...
  onReveal = () => {
    this.setState(({ showFront, showAnswers }) => ({
  ...
  handleHotKeyUp = (keyName, e, handle) => {
    switch(keyName) {
      case "space":
        this.onReveal();
        break;
      case "1":
        if (this.state.showAnswers === true)
          this.onReview(null, {value: REVIEW_TYPE.REDO})
        break;
      case "2":
        if (this.state.showAnswers === true)
          this.onReview(null, {value: REVIEW_TYPE.HARD})
        break;
      case "3":
        if (this.state.showAnswers === true)
          this.onReview(null, {value: REVIEW_TYPE.EASY})
        break;
      default:
        break;
    }
