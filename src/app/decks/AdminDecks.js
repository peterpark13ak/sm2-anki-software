...
class AdminDecks extends Component<Props, State> {
  ...
  fetchDecks = () => {
    api.fetchAllDecks().then(response => {
  ...
  render() {
    const { decks = [], filter, isLoading, users=[] } = this.state;
              ...
              <Grid divided>
                <Grid.Row>
                  ...
                  {!isLoading && filteredDecks.length > 0 ? (
                    <div>
                      {users.map((user, idx) => (user.email !== ADMIN_ID &&
                        <div key={idx}>
                          <h3 style={{marginTop: 40}}>{`${user.name}(${user.email})`}</h3>
                          <div className="row">
                            {filteredDecks.map((deck, key) => (deck.user === user._id) && (
                              <DeckCard
...
export default withErrors(AdminDecks);