...
class DeckHome extends Component {
  ...
  fetchDeck = deckId => {
    api.fetchDeck(deckId).then(
      response => {
        api.fetchDeck(response.data.parentDeck).then(
          res => {
            this.setState({ parentDeck: res.data })
          }
        )
  ...
  render() {
    const { deck, parentDeck, cards, showModalType, isLoading, isError } = this.state;
                ...
                panes={[
                  {
                    ...
                    render: () => <OverviewTab deck={deck} parentDeck={parentDeck} />,
                  },
                  {
